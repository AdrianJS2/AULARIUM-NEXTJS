// RUTA: app/api/asignar-aulas/septiembre-diciembre/route.ts

import { NextResponse } from "next/server";
import pool from "@/lib/db";
// HE AQUÍ LA CORRECCIÓN: Se importa desde el archivo de servidor 'auth-server'.
import { isAdmin, getUserRole } from "@/lib/auth-server";
import type { NextRequest } from "next/server";

// Lógica de asignación (reutilizada)
...
export async function POST(request: NextRequest) {
    ...
    try {
        // HE AQUÍ LA CORRECCIÓN: Se recibe el 'userId' desde el frontend.
        ...
        // Se usan los helpers del servidor.
        const { rol, carrera_id } = await getUserRole(userId);
        const admin = await isAdmin(userId);
        
        const tables = { materias: "materias_septiembre_diciembre", grupos: "grupos_septiembre_diciembre", asignaciones: "asignaciones_septiembre_diciembre" };
        ...
        await connection.rollback();
        console.error("Error en asignación de aulas (Septiembre-Diciembre):", error);
